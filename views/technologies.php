<div class="container">
    <div class="row">
        <div class="col-lg-12 text-center">
            <h2 class="section-heading">Languages</h2>
            <hr class="primary-divider">
        </div>
    </div>
</div>
<div class="container">
    <div class="row">
        <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="icon-javascript languages-icon" style="margin-left:40px;"></i>
                <h3>JavaScript</h3>
            </div>
        </div>
        <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="icon-php languages-icon"></i>
                <h3>PHP</h3>
                
            </div>
        </div>
        <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="icon-mysql languages-icon"></i>
                <h3>MySQL</h3>
            </div>
        </div>
        <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="fa fa-file-code-o languages-icon" style="margin-top: 40px;"></i>
                <h3>ColdFusion</h3>
            </div>
        </div>

        <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="fa fa-html5 languages-icon"></i>
                <h3>HTML5</h3>
            </div>
        </div>
        <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="fa fa-css3 languages-icon"></i>
                <h3>CSS3</h3>
                
            </div>
        </div>
        <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="icon-java-bold languages-icon"></i>
                <h3>Java</h3>
            </div>
        </div>
        <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="fa fa-file-excel-o languages-icon" style="margin-top: 40px;"></i>
                <h3>XML</h3>
            </div>
        </div>
    </div>
</div>

<div class="container" style="margin-top:100px;">
    <div class="row">
        <div class="col-lg-12 text-center">
            <h2 class="section-heading">Platforms</h2>
            <hr class="primary-divider">
        </div>
    </div>
</div>
<div class="container">
    <div class="row">
         <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="icon-javascript languages-icon" style="margin-top: 40px;"></i>
                <h3>NodeJs</h3>
            </div>
        </div>
        <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="fa fa-android languages-icon" style="margin-top: 40px;"></i>
                <h3>Android</h3>
            </div>
        </div>
        <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="fa fa-apple languages-icon" style="margin-top: 40px;"></i>
                <h3>iOS</h3>
                
            </div>
        </div>
        <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="fa fa-apple languages-icon" style="margin-top: 40px;"></i>
                <h3>Mac</h3>
            </div>
        </div>
        <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="icon-html languages-icon" style="margin-left: 35px;"></i>
                <h3>web</h3>
            </div>
        </div>
        <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="icon-raspberrypi languages-icon" style="margin-left: 40px;"></i>
                <h3>raspberry pi</h3>
                
            </div>
        </div>
        <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="icon-ubuntu languages-icon"></i>
                <h3>linux</h3>
            </div>
        </div>
        <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="icon-apache languages-icon"></i>
                <h3>apache</h3>
            </div>
        </div>
    </div>
</div>

<div class="container" style="margin-top:100px;">
    <div class="row">
        <div class="col-lg-12 text-center">
            <h2 class="section-heading">Frameworks</h2>
            <hr class="primary-divider">
        </div>
    </div>
</div>
<div class="container">
    <div class="row">
        <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="icon-laravel languages-icon"></i>
                <h3>Laravel</h3>
            </div>
        </div>
        <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="fa fa-eye languages-icon" style="margin-top: 40px;"></i>
                <h3>Ionic</h3>
                
            </div>
        </div>
        <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="fa fa-bold languages-icon" style="margin-top: 40px;"></i>
                <h3>Bootstrap</h3>
            </div>
        </div>
        <div class="col-lg-3 col-md-4 col-sm-4 col-sm-offset-0 col-xs-6 col-xs-offset-0 text-center">
            <div class="service-box service-box-mod">
                <i class="fa fa-book languages-icon" style="margin-top: 40px;"></i>
                <h3>Much More</h3>
            </div>
        </div>
    </div>
</div>

